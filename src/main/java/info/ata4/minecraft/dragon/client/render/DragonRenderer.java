...
public class DragonRenderer extends RenderLiving {
    ...
    private final Map<DragonBreed, DragonModel> breedModels = new HashMap<>();
    ...
    public void doRender(EntityTameableDragon dragon, double x, double y, double z, float yaw, float partialTicks) {
        ...
        if (dragon.isEgg()) {
            renderEgg(dragon, x, y, z, partialTicks);
        } else {
    ...
    protected void renderEgg(EntityTameableDragon dragon, double x, double y, double z, float partialTicks) {
        // apply egg wiggle
    ...
    public int shouldRenderPass(EntityLivingBase entity, int pass, float scale) {
        ...
        switch (pass) {
            // pass 1 - saddle
            case 0 -> {
                if (((EntityTameableDragon)entity).isSaddled()) {
    ...
    protected void preRenderCallback(EntityLivingBase entity, float partialTicks) {
        float scale = ((EntityTameableDragon)entity).getScale() * 0.8f;
    ...
    protected void passSpecialRender(EntityLivingBase entity, double x, double y, double z) {
        ...
        // lighting when using model multi-pass rendering
    }
